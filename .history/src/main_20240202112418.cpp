...
void loop() {
  ...
  float current_position = motor.shaft;

  // 计算阻尼力
  float damping_torque = -torque_controller.PID_velocity.P * current_position;

  // 设置阻尼力作为目标电流
  motor.move(damping_torque);
  ...
  Serial.println(motor.torque_controller->target);
